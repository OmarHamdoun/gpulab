...
float FlowLibGpuSOR::computeFlow()
{
	...
	for( rec_depth = max_rec_depth; rec_depth >= 0; --rec_depth )
	{
		...
		// bind resampled images to texture memory
		...
		{
			// warp image 2 by resized flow field
			...
			// reset flow increment du/dv to zero
